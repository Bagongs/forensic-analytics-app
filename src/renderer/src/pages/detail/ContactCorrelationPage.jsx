...
// src/renderer/src/pages/detail/ContactCorrelationPage.jsx
...
const ROW_HEIGHT = 48
const OVERSCAN = 10
const SKELETON_DEVICE_COLS = 4 // placeholder jumlah kolom device saat loading
...
function SkeletonRow({ style, gridTemplateColumns, stickyBg }) {
  // robust: hitung jumlah kolom dari template dengan split whitespace
  const colCount = gridTemplateColumns.split(/\s+/).filter(Boolean).length
  return (
    <div
      className="grid items-center px-4 border-t border-[#293240] animate-pulse"
      style={{ ...style, gridTemplateColumns }}
    >
      {/* sticky first cell skeleton */}
...
/* ====================== Page ====================== */
export default function ContactCorrelationPage() {
  ...
  useEffect(() => {
    ...
    const fetchNow = async () => {
      ...
      setError('')
      try {
        const res = await window.api.analytics.getContactCorrelation({ analytic_id: analysisId })
        if (!mounted) return
  ...
  // === Baris: setiap contact number ===
  const rows = useMemo(() => {
    const base = data?.correlations || []
    return base.map((c) => {
      const map = new Map()
      for (const x of c.devices_found_in || []) {
        map.set(x.device_label, x.contact_name || '-')
      }
      return {
  ...
  // Export guard (disable saat loading/empty/exporting)
  ...
  const onExportPdf = async () => {
    ...
    await exportPdfWithToast({
      ...
      fileName: data?.analytic_name || 'contact-correlation-report',
      setIsExporting
    })
  }
  ...
  const GRID_TEMPLATE = useMemo(() => {
    // Kolom pertama (Number) lebar, sisanya fix 160px
    const repeat = visibleLabels.map(() => '160px').join(' ')
    return `${Math.max(34, 28)}% ${repeat}`
  }, [visibleLabels])

  // Grid template khusus skeleton saat loading (Number + 4 device)
  const GRID_TEMPLATE_SKELETON = useMemo(() => {
    const repeat = Array.from({ length: SKELETON_DEVICE_COLS }, () => '160px').join(' ')
    return `${Math.max(34, 28)}% ${repeat}`
  }, [])
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              gridTemplateColumns: loading ? GRID_TEMPLATE_SKELETON : GRID_TEMPLATE,
              height: 52,
              position: 'sticky',
              top: 0,
              zIndex: 30
          ...
          >
            ...
            {(loading ? Array.from({ length: SKELETON_DEVICE_COLS }) : visibleLabels).map(
              (lbl, i) => (
                <div key={i} className="text-sm text-left font-[Aldrich] text-[#EDC702]">
                  {loading ? `Device ${i + 1}` : lbl}
                </div>
              )
            )}
          </div>
          ...
          <div
            ...
            style={{ maxHeight: 520, minHeight: 260, willChange: 'transform' }}
          >
            {/* Saat loading, gunakan tinggi tetap (viewport) agar UI tidak “lompat” */}
            <div style={{ height: loading ? viewportH : totalHeight, position: 'relative' }}>
              ...
              {/* Loading skeleton rows (isi viewport) */}
              {!error && loading && (
                <>
                    ...
                    (_, i) => {
                      ...
                      return (
                        <SkeletonRow
                          ...
                          gridTemplateColumns={GRID_TEMPLATE_SKELETON} // gunakan template skeleton
              ...
              {!loading && !error && rows.length === 0 && (
                <div
                  className="absolute inset-0 grid place-items-center text-center opacity-70"
                  style={{
                    minHeight: viewportH || 520,
                    background:
                      'linear-gradient(to bottom, rgba(11,15,23,0.95) 0%, rgba(11,15,23,0.98) 100%)'
                  }}
                >
                  <div className="text-sm text-[#E7E9EE] font-[Noto Sans]">
                    No contact correlation data.
                  </div>
                </div>
              )}
              ...
              {!loading && !error && rows.length > 0 && (
                <>
                  {rows.slice(start, end).map((row, i) => {
                    const rIdx = start + i
                    const rowBg = rIdx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)'
                    return (
                      <div
                        key={rIdx}
                      ...
                      >
                        {/* sticky first cell */}
                        <div
                          className="truncate sticky left-0 z-20"
                        ...
                        {/* names per device (sesuai window tabs) */}
                        {visibleLabels.map((lbl, j) => {
                          const name = row.labelToName.get(lbl) || '-'
                          return (
                            <div key={j} className="px-2 truncate" title={name}>
                              {name}
                            </div>
                          )
                        })}
